...
marginal_transformer <- function(df, metadata, var, q,
  ...
  log_debug(paste0("Length var: ", length(df[[var]])))
  ...
  log_debug(paste0("Line 414: Extracted ", nrow(df), " rows for variable ", var))
  ...
  fit_gridchunk <- function(i) {
    ...
    log_debug(paste0("stats:478: Shape of df: ", dim(df)))
    log_debug(paste0("stats:479: Shape of meta: ", dim(meta)))
    ...
    log_debug(paste0("stats:490: Shape of maxima: ", dim(maxima)))
  }
  ...
  log_debug(paste0("stats:539: Shape of transformed: ", dim(transformed)))
