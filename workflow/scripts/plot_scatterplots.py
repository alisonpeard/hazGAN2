...
def op2idx(ops:dict, data:np.ndarray, extent:list):
    ...
    coords = np.array([Point(lon, lat) for lat in lats for lon in lons])
...
def main(input, output, params):
    ...
    pixels = list(ops.values())[:2]
    labels = list(ops.keys())[:2]
    ...
    for i, _field in enumerate(_fields):
        fig = plotting.scatter.plot(gener_x, train_x, field=i, pixels=pixels, s=10,
                        cmap=params.cmap, xlabel=labels[0].capitalize(), 
                        ylabel=labels[1].capitalize())
