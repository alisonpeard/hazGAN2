...
compare to what I currently have (switched to the above) (May 2025):
```bash
export PATH=/usr/local/$CUDAVAR/bin:$PATH
export LD_LIBRARY_PATH=/usr/local/$CUDAVAR/lib:$LD_LIBRARY_PATH
export LD_LIBRARY_PATH=/usr/local/$CUDAVAR/lib64:$LD_LIBRARY_PATH
export CUDA_PATH=/usr/local/$CUDAVAR
export CUDA_ROOT=/usr/local/$CUDAVAR
export CUDA_HOME=/usr/local/$CUDAVAR
export CUDA_HOST_COMPILER=/usr/bin/
```
